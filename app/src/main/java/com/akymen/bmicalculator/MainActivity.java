package com.akymen.bmicalculator;
...
public class MainActivity extends AppCompatActivity {
    ...
    //method to assign id of views to class variables
    ...
    private double calculateBMI() {
        ...
        //Convert to strings to integer vars
        int feet = Integer.parseInt(feetText);
        int inches = Integer.parseInt(inchesText);
        int weight = Integer.parseInt(weightText);

        //Conversion of units feet to inches to meters
    ...
    private void displayResult(double bmi) {
        //formatting decimal to two places
        DecimalFormat myDecimal = new DecimalFormat("0.00");
        String bmiTextResult = myDecimal.format(bmi);

        String fullResult;
        //Display underweight, overweight and healthy respectively
        if(bmi < 18.5){
            fullResult = bmiTextResult + " - You are underweight";
        } else if(bmi > 25){
            fullResult = bmiTextResult + " - You are overweight";
        } else{
            fullResult = bmiTextResult + " - You are healthy";
        }
